...
class Corpus(object):
    ...
    # parse train docs
    ##################
    def parse_raw_data(self, pairs_file, labels_file):
    ...
    def split_data(self):
        ...
        # n = int(0.8 * len(train_fandoms))
        # fandoms_train = train_fandoms[:n]
        # fandoms_cal = train_fandoms[n:]
        # fandoms_val = list(self.test_fandoms)
        # fandoms_train = ['A', 'B']
        # fandoms_cal = ['A', 'B']
        # fandoms_val = ['A', 'B']
        fandoms_train = fandoms_cal = fandoms_val = list(self.fandoms)
        ...
        shuffled_authors = shuffle(list(self.authors))
        idx_a = int(len(shuffled_authors) * 0.8)
        idx_b = int(len(shuffled_authors) * 0.9)
        
        authors_train = shuffled_authors[:idx_a]
        authors_cal = shuffled_authors[idx_a:idx_b]
        authors_val = shuffled_authors[idx_b:]
...
def run(model_folder):
    ...
    ########################################################################
    pairs_file = os.path.join(model_folder, 'train_pairs.jsonl')
    labels_file = os.path.join(model_folder, 'train_labels.jsonl')
    ...
    corpus.parse_raw_data(pairs_file, labels_file)
