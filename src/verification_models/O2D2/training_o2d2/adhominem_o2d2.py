...
from verification_models.O2D2.training_o2d2.resample_pairs_cal import resample_pairs_cal
...
class AdHominem_O2D2():
    ...
    def __init__(self, hyper_parameters, theta_init, theta_E_init):
        ...
        ############################
        with tf.compat.v1.variable_scope('deep_metric_learning'):
        ...
        ###################################################
        with tf.compat.v1.variable_scope('Euclidean_distance_and_kernel_function'):
        ...
        ##################################
        with tf.compat.v1.variable_scope('Bayes_factor_scoring'):
        ...
        ####################################
        with tf.compat.v1.variable_scope('uncertainty_adaptation_layer'):
        ...
        #####################################
        with tf.compat.v1.variable_scope('out_of_distribution_detector'):
    ...
    def o2d2(self, lev_L, lev_R, conf_matrix, theta):
        ...
        y = tf.compat.v1.layers.dropout(y, rate=self.hyper_parameters['rate_o2d2'], training=is_training)
        y = tf.compat.v1.nn.xw_plus_b(y, theta["W1"], theta["b1"])
        y = tf.nn.tanh(y)

        # layer 2
        y = tf.compat.v1.layers.dropout(y, rate=self.hyper_parameters['rate_o2d2'], training=is_training)
        y = tf.compat.v1.nn.xw_plus_b(y, theta["W2"], theta["b2"])
        y = tf.nn.tanh(y)

        # layer 3
        y = tf.compat.v1.layers.dropout(y, rate=self.hyper_parameters['rate_o2d2'], training=is_training)
        logits = tf.compat.v1.nn.xw_plus_b(y, theta["W3"], theta["b3"])
    ...
    def get_o2d2_labels(self, pred_ual, labels, thr_0=0.45, thr_1=0.55):
        ...
        condition_0 = tf.compat.v1.where(tf.greater(thr_0, pred_ual),
                               tf.zeros_like(pred_ual, dtype=tf.float32),
                               tf.ones_like(pred_ual, dtype=tf.float32),
                               )
        condition_1 = tf.compat.v1.where(tf.less(thr_1, pred_ual),
                               tf.zeros_like(pred_ual, dtype=tf.float32),
                               tf.ones_like(pred_ual, dtype=tf.float32),
                               )
        condition = tf.multiply(condition_0, condition_1)

        labels_o2d2 = tf.compat.v1.where(tf.cast(condition, dtype=tf.bool),
                               condition,
                               labels_o2d2,
                               )
        return labels_o2d2
    ...
    def metric_layer(self, e_d, dropout, theta):
        ...
        y = tf.compat.v1.nn.xw_plus_b(y, theta['W'], theta['b'])
        # nonlinear output
        y = tf.nn.tanh(y)
    ...
    def initialize_placeholders(self, theta_E):
        ...
        ####################
        # labels
        labels = tf.compat.v1.placeholder(tf.float32, shape=[None], name='labels')
    ...
    def initialize_o2d2(self, D_lev):
        ...
        theta = {"W1": tf.compat.v1.get_variable(name="W1",
                 ...
                 "b2": tf.compat.v1.get_variable(name="b2",
                                       ...
                                       trainable=True,
                                       dtype=tf.float32),
                 "W3": tf.compat.v1.get_variable(name="W3",
                                       shape=[30, 1],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"),
                                       trainable=True,
                                       dtype=tf.float32,
                                       ),
                 "b3": tf.compat.v1.get_variable(name="b3",
                                       shape=[1],
                                       initializer=tf.compat.v1.constant_initializer(0.),
                                       trainable=True,
                                       dtype=tf.float32),
    ...
    # train end-to-end
    ##################
    def train_model(self, model_folder, val_set, file_results, file_tmp, dir_results):
        ...
        for epoch in range(epochs):
            ...
            docs_L_tr, docs_R_tr, labels_tr = resample_pairs_cal(model_folder, file_results)
            # shuffle (again)
