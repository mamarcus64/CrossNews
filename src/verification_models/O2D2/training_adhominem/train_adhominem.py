# -*- coding: utf-8 -*-
from verification_models.O2D2.training_adhominem.adhominem import AdHominem
import pickle
import os
...
def run(model_folder, hyper_parameters):
    
    # create folder for results
    dir_results = os.path.join(model_folder, 'results_adhominem')
    if not os.path.exists(dir_results):
        os.makedirs(dir_results)

    # load dev set
    with open(os.path.join(model_folder, 'data_preprocessed', "pairs_dev"), 'rb') as f:
    ...
    path_wrd = os.path.join(model_folder, 'data_preprocessed',
    ...
    path_chr = os.path.join(model_folder, 'data_preprocessed', 'vocab_chr_' + str(hyper_parameters['num_chr_embeddings']))
    ...
    # start training
    adhominem.train_model(model_folder, dev_set, file_results, file_tmp, dir_results)
    # close session
