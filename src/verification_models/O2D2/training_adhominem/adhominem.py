...
from verification_models.O2D2.training_adhominem.resample_pairs_train import resample_pairs_train
...
class AdHominem():
    ...
    def __init__(self, hyper_parameters, E_w_init):
        ...
        # batch size
        self.B = tf.shape(input=self.placeholders['e_w_L'])[0]
        ...
        ##########################################
        with tf.compat.v1.variable_scope('feature_extraction_doc2vec'):
        ...
        ############################
        with tf.compat.v1.variable_scope('deep_metric_learning_a'):
            self.lev_a_L, self.lev_a_R = self.metric_layer(e_d,
                                                           self.dropout['metric_a'],
                                                           self.theta['metric_a'],
                                                           stop_gradient=False,
                                                           )
        with tf.compat.v1.variable_scope('deep_metric_learning_f'):
        ...
        ###################################################
        with tf.compat.v1.variable_scope('Euclidean_distance_and_kernel_function_a'):
        ...
        with tf.compat.v1.variable_scope('Euclidean_distance_and_kernel_function_f'):
        ...
        ##################################
        with tf.compat.v1.variable_scope('Bayes_factor_scoring_a'):
        ...
        with tf.compat.v1.variable_scope('Bayes_factor_scoring_f'):
    ...
    def ual_layer(self, theta, dropout, pred_bfs, lev_L, lev_R, stop_gradient=True):
        ...
        y = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                    true_fn=lambda: tf.multiply(dropout['ual_1'], y),
                    false_fn=lambda: y,
                    )

        # dense layer
        y = tf.compat.v1.nn.xw_plus_b(y, theta["W1"], theta["b1"])
        y = tf.nn.tanh(y)

        # apply dropout
        y = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                    true_fn=lambda: tf.multiply(dropout['ual_2'], y),
                    false_fn=lambda: y,
                    )

        # compute confusion matrix, shape = [B, 2 * 2]
        logits = tf.compat.v1.nn.xw_plus_b(y, theta["W2"], theta["b2"])
        # shape = [B, 2, 2]
        ...
        reg = tf.reduce_sum(input_tensor=tf.multiply(tf.math.log(p), p), axis=1)
        reg = tf.reduce_mean(input_tensor=reg)

        # loss
        idx = tf.stack([tf.range(0, self.B, 1), labels], axis=1)
        loss_ual = tf.gather_nd(pred_ual, idx)
        loss_ual = tf.reduce_mean(input_tensor=-tf.math.log(loss_ual + 1e-8))
    ...
    def prepare_optimizer(self):
        ...
        # optimizer
        opt = tf.compat.v1.train.AdamOptimizer(learning_rate=self.placeholders['lr'])
    ...
    def metric_layer(self, e_d, dropout, theta, stop_gradient=False):
        ...
        # apply dropout
        y = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                    true_fn=lambda: tf.multiply(dropout_mask, e_d),
                    false_fn=lambda: e_d,
                    )
        # fully-connected layer
        y = tf.compat.v1.nn.xw_plus_b(y,
                            theta['W'],
                            theta['b'],
                            )
        # nonlinear output
    ...
    def bfs_layer(self, lev_L, lev_R, theta_bfs_1, theta_bfs_2, dropout, stop_gradient=True):
        ...
        def makeS(C, D):
            diagC = tf.math.exp(tf.linalg.tensor_diag_part(C))
            maskC = tf.ones_like(C) - tf.eye(D)
            maskC = tf.linalg.band_part(maskC, 0, -1)
            C = tf.multiply(maskC, C) + tf.linalg.tensor_diag(diagC)
        ...
        # apply dropout
        is_training = self.placeholders['is_training']
        x1 = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                     true_fn=lambda: tf.multiply(dropout['bfs_1'], lev_L),
                     false_fn=lambda: lev_L,
                     )
        x2 = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                     true_fn=lambda: tf.multiply(dropout['bfs_1'], lev_R),
                     false_fn=lambda: lev_R,
                     )

        # fully-connected layer
        x1 = tf.compat.v1.nn.xw_plus_b(x1,
                             theta_bfs_1['W'],
                             theta_bfs_1['b'],
                             )
        x2 = tf.compat.v1.nn.xw_plus_b(x2,
                             theta_bfs_1['W'],
                             theta_bfs_1['b'],
                             )
        x1 = tf.nn.relu(x1)
        ...
        A_tilde = tf.linalg.inv(B + 2 * W + 1e-6 * tf.eye(self.hyper_parameters['D_plda']))
        G_tilde = tf.linalg.inv(B + W + 1e-6 * tf.eye(self.hyper_parameters['D_plda']))
        ...
        # compute entropy
        H_W = tf.linalg.logdet(tf.linalg.inv(W))
        H_B = tf.linalg.logdet(tf.linalg.inv(B))
    ...
    def initialize_placeholders(self, E_w_init):
        ...
        x_c_L = tf.compat.v1.placeholder(dtype=tf.int32,
                               shape=[None, T_s, T_w, T_c],
                               name='x_c_L',
                               )
        x_c_R = tf.compat.v1.placeholder(dtype=tf.int32,
                               shape=[None, T_s, T_w, T_c],
                               name='x_c_R',
                               )

        # initialize embedding matrix for characters
        with tf.compat.v1.variable_scope('character_embedding_matrix'):
            # zero-padding embedding
            E_c_0 = tf.zeros(shape=[1, D_c], dtype=tf.float32)
            # trainable embeddings
            E_c_1 = tf.compat.v1.get_variable(name='E_c_1',
                                    shape=[len(V_c) - 1, D_c],
                                    initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                    trainable=True,
                                    dtype=tf.float32,
                                    )
        ...
        e_c_L = tf.nn.embedding_lookup(params=E_c, ids=x_c_L)
        e_c_R = tf.nn.embedding_lookup(params=E_c, ids=x_c_R)

        # word-based placeholder for two documents
        x_w_L = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, T_s, T_w], name='x_w_L')
        x_w_R = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, T_s, T_w], name='x_w_R')

        # true sentence / document lengths
        N_w_L = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, T_s], name='N_w_L')
        N_w_R = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, T_s], name='N_w_R')
        N_s_L = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None], name='N_s_L')
        N_s_R = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None], name='N_s_R')

        # matrix for word embeddings, shape=[len(V_w), D_w]
        with tf.compat.v1.variable_scope('word_embedding_matrix'):
            # zero-padding embedding
        ...
        e_w_L = tf.nn.embedding_lookup(params=E_w, ids=x_w_L)
        e_w_R = tf.nn.embedding_lookup(params=E_w, ids=x_w_R)

        ####################
        # training variables
        ####################
        # labels
        labels_a = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name='labels_a')
        labels_f = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name='labels_f')

        # training mode (for dropout regularization)
        is_training = tf.compat.v1.placeholder(dtype=tf.bool, name='training_mode')
        # learning rate
    ...
    def feature_extraction(self, e_c, e_w, N_w, N_s):
        ...
        with tf.compat.v1.variable_scope('characters_to_word_encoding'):
            r_c = self.cnn_layer_cw(e_c)
        with tf.compat.v1.variable_scope('words_to_sentence_encoding'):
            e_cw = tf.concat([e_w, r_c], axis=3)
            h_w = self.bilstm_layer_ws(e_cw, N_w)
            e_s = self.att_layer_ws(h_w, N_w)
        with tf.compat.v1.variable_scope('sentences_to_document_encoding'):
    ...
    def compute_kernel_distance(lev_L, lev_R, alpha, beta):
        ...
        # shape = (B, 1)
        distance = tf.reduce_sum(input_tensor=distance, axis=1, keepdims=True) + 1e-8
        # kernel
    ...
    def cnn_layer_cw(self, e_c):
        ...
        # dropout
        e_c = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                      true_fn=lambda: tf.multiply(dropout_mask, e_c),
                      false_fn=lambda: e_c,
                      )
        # zero-padding, shape = [B * T_s * T_w, T_c + 2 * (h-1), D_c]
        e_c = tf.pad(tensor=e_c,
                     paddings=tf.constant([[0, 0], [h - 1, h - 1], [0, 0]]),
                     mode='CONSTANT',
                     )

        # 1D convolution
        # shape = [B * T_s * T_w, T_c + 2 * (h-1) - h + 1, D_r] = [B * T_s * T_w, T_c + h - 1, D_r]
        r_c = tf.nn.conv1d(input=e_c,
                           filters=self.theta['cnn']['W'],
                           stride=1,
                           padding='VALID',
        ...
        r_c = tf.nn.max_pool2d(input=r_c,
                             ksize=[1, T_c + h - 1, 1, 1],
                             strides=[1, 1, 1, 1],
                             padding='VALID',
                             )
    ...
    def bilstm_layer_ws(self, e_w_f, N_w):
        ...
        e_w_b = tf.reverse_sequence(input=e_w_f, seq_lengths=N_w, seq_axis=1)
        ...
        states = tf.scan(self.bilstm_cell_ws,
                         [tf.transpose(a=e_w_f, perm=[1, 0, 2]),
                          tf.transpose(a=e_w_b, perm=[1, 0, 2]),
                          tf.transpose(a=N_w_t, perm=[1, 0])],
        ...
        h_f = states[0]
        h_b = states[1]
        h_f = tf.transpose(a=h_f, perm=[1, 0, 2])
        h_b = tf.transpose(a=h_b, perm=[1, 0, 2])
        # reverse again backward state
        h_b = tf.reverse_sequence(input=h_b, seq_lengths=N_w, seq_axis=1)
    ...
    def bilstm_cell_ws(self, prev, input):
        ...
        h_next_f = tf.compat.v1.where(condition, h_next_f, h_prev_f)
        c_next_f = tf.compat.v1.where(condition, c_next_f, c_prev_f)
        h_next_b = tf.compat.v1.where(condition, h_next_b, h_prev_b)
        c_next_b = tf.compat.v1.where(condition, c_next_b, c_prev_b)
    ...
    def bilstm_layer_sd(self, e_s_f, N_s):
        ...
        # reverse input sentences
        e_s_b = tf.reverse_sequence(input=e_s_f, seq_lengths=N_s, seq_axis=1)
        ...
        states = tf.scan(self.bilstm_cell_sd,
                         [tf.transpose(a=e_s_f, perm=[1, 0, 2]),
                          tf.transpose(a=e_s_b, perm=[1, 0, 2]),
                          tf.transpose(a=N_s_t, perm=[1, 0])],
                         initializer=[h_0_f, h_0_b, c_0_f, c_0_b, t_0],
                         name='lstm_sd_layer',
                         )
        ...
        h_b = tf.transpose(a=h_b, perm=[1, 0, 2])
        # reverse again backward state
        h_b = tf.reverse_sequence(input=h_b, seq_lengths=N_s, seq_axis=1)
    ...
    def lstm_cell(self, e_w, h_prev, c_prev, params, dropout):
        ...
        e_w = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                      true_fn=lambda: tf.multiply(dropout_x, e_w),
                      false_fn=lambda: e_w,
                      )
        h_prev = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                         true_fn=lambda: tf.multiply(dropout_h, h_prev),
    ...
    def att_layer_ws(self, h_w, N_w):
        ...
        scores = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                         true_fn=lambda: tf.multiply(dropout_Wb, h_w),
                         false_fn=lambda: h_w,
                         )
        # shape=[2 * B * T_s * T_w, 2 * D_s]
        scores = tf.reshape(scores, shape=[2 * self.B * T_s * T_w, 2 * D_s])
        scores = tf.nn.tanh(tf.compat.v1.nn.xw_plus_b(scores,
                                            self.theta['att_ws']['W'],
                                            self.theta['att_ws']['b']))
        ...
        # apply dropout
        scores = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                         true_fn=lambda: tf.multiply(dropout_v, scores),
                         false_fn=lambda: scores,
                         )
        # shape=[2 * B * T_s * T_w, 2 * D_s]
        scores = tf.reshape(scores, shape=[2 * self.B * T_s * T_w, 2 * D_s])
        ...
        e_s = tf.reduce_sum(input_tensor=tf.multiply(alpha, h_w), axis=2, keepdims=False)
    ...
    def att_layer_sd(self, h_s, N_s):
        ...
        scores = tf.cond(pred=tf.equal(is_training, tf.constant(True)),
                         true_fn=lambda: tf.multiply(dropout_Wb, h_s),
                         false_fn=lambda: h_s,
                         )
        # shape=[2 * B * T_s, 2 * D_d]
        ...
        e_d = tf.reduce_sum(input_tensor=tf.multiply(alpha, h_s), axis=1, keepdims=False)
    ...
    def make_dropout_mask(self, shape, keep_prob):
        keep_prob = tf.convert_to_tensor(value=keep_prob, dtype=tf.float32)
        random_tensor = keep_prob + tf.random.uniform(shape, dtype=tf.float32)
    ...
    def initialize_dropout(self):
        ...
        with tf.compat.v1.variable_scope('dropout_lstm_ws_backward'):
        ...
        with tf.compat.v1.variable_scope('dropout_lstm_sd_forward'):
        ...
        with tf.compat.v1.variable_scope('dropout_lstm_sd_backward'):
        ...
        with tf.compat.v1.variable_scope('dropout_att_ws'):
            dropout['att_ws'] = {}
        ...
        with tf.compat.v1.variable_scope('dropout_att_sd'):
            dropout['att_sd'] = {}
    ...
    def initialize_parameters(self):
        ...
        with tf.compat.v1.variable_scope('theta_cnn'):
            theta['cnn'] = self.initialize_cnn(D_c, D_r, h)

        with tf.compat.v1.variable_scope('theta_lstm_ws_forward'):
            theta['lstm_ws_forward'] = self.initialize_lstm(D_w + D_r, D_s)
        with tf.compat.v1.variable_scope('theta_lstm_ws_backward'):
            theta['lstm_ws_backward'] = self.initialize_lstm(D_w + D_r, D_s)

        with tf.compat.v1.variable_scope('theta_lstm_sd_forward'):
            theta['lstm_sd_forward'] = self.initialize_lstm(2 * D_s, D_d)
        with tf.compat.v1.variable_scope('theta_lstm_sd_backward'):
            theta['lstm_sd_backward'] = self.initialize_lstm(2 * D_s, D_d)

        with tf.compat.v1.variable_scope('theta_att_ws'):
            theta['att_ws'] = self.initialize_att(2 * D_s, 2 * D_s)
        with tf.compat.v1.variable_scope('theta_att_sd'):
            theta['att_sd'] = self.initialize_att(2 * D_d, 2 * D_d)

        with tf.compat.v1.variable_scope('theta_metric_a'):
            theta['metric_a'] = self.initialize_mlp(2 * D_d, D_lev)
        with tf.compat.v1.variable_scope('theta_metric_f'):
            theta['metric_f'] = self.initialize_mlp(2 * D_d, D_lev)

        with tf.compat.v1.variable_scope('theta_bfs_a_1'):
            theta['bfs_a_1'] = self.initialize_mlp(D_lev, D_plda)
        with tf.compat.v1.variable_scope('theta_bfs_a_2'):
            theta['bfs_a_2'] = self.initialize_bfs(D_plda)

        with tf.compat.v1.variable_scope('theta_bfs_f_1'):
            theta['bfs_f_1'] = self.initialize_mlp(D_lev, D_plda)
        with tf.compat.v1.variable_scope('theta_bfs_f_2'):
            theta['bfs_f_2'] = self.initialize_bfs(D_plda)

        with tf.compat.v1.variable_scope('theta_ual_a'):
            theta['ual_a'] = self.initialize_ual(D_lev)

        with tf.compat.v1.variable_scope('theta_dml_loss_a'):
            theta['loss_dml_a'] = self.initialize_theta_loss()

        return theta

    def initialize_theta_loss(self):
        theta = {'alpha': tf.compat.v1.get_variable(name='loss_alpha',
                                          shape=[1],
                                          initializer=tf.compat.v1.constant_initializer(0.09),
                                          trainable=self.hyper_parameters["train_kernel"],
                                          dtype=tf.float32,
                                          ),
                 'beta': tf.compat.v1.get_variable(name='loss_beta',
                                         shape=[1],
                                         initializer=tf.compat.v1.constant_initializer(3.0 / 2.0),
                                         trainable=self.hyper_parameters["train_kernel"],
                                         dtype=tf.float32,
                                         ),
                 }
    ...
    def initialize_ual(self, D):
        theta = {'W1': tf.compat.v1.get_variable(name='W1',
                                       shape=[D, 2*D],
    ...
    def initialize_bfs(self, D):
        ...
        theta = {'W': tf.Variable(np.array(m_W, dtype=np.float32), name='W', trainable=True),
                 ...
                 'mu': tf.compat.v1.get_variable(name='mu',
                                       shape=[D, 1],
                                       initializer=tf.compat.v1.initializers.random_uniform(minval=-0.0001, maxval=0.0001),
                                       trainable=True,
                                       ),
                 }
        return theta
    ...
    def initialize_mlp(self, D_in, D_out):
        r = 0.4
        theta = {'W': tf.compat.v1.get_variable('W',
                                      shape=[D_in, D_out],
                                      initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                      trainable=True,
                                      dtype=tf.float32,
    ...
    def initialize_cnn(self, D_in, D_out, h):
        r = 0.1
        theta = {'W': tf.compat.v1.get_variable(name='W',
                                      shape=[h, D_in, D_out],
                                      initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                      trainable=True,
                                      dtype=tf.float32,
    ...
    def initialize_att(self, D_in, D_out):
        r = 0.03
        theta = {'W': tf.compat.v1.get_variable('W_a',
                                      shape=[D_in, D_out],
                                      initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                      trainable=True,
                                      dtype=tf.float32,
                                      ),
                 'v': tf.compat.v1.get_variable('v_a', shape=[D_out, 1],
                                      initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                      trainable=True,
                                      dtype=tf.float32,
                                      ),
                 'b': tf.compat.v1.get_variable('b_a', shape=[D_out],
                                      initializer=tf.compat.v1.constant_initializer(0.0),
                                      trainable=True,
                                      dtype=tf.float32,
                                      ),
    ...
    def initialize_lstm(self, D_in, D_out):
        r = 0.05
        theta = {'W_i': tf.compat.v1.get_variable('W_i', shape=[D_in, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'U_i': tf.compat.v1.get_variable('U_i', shape=[D_out, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'b_i': tf.compat.v1.get_variable('b_i', shape=[1, D_out],
                                        initializer=tf.compat.v1.constant_initializer(0.0),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'W_f': tf.compat.v1.get_variable('W_f', shape=[D_in, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'U_f': tf.compat.v1.get_variable('U_f', shape=[D_out, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'b_f': tf.compat.v1.get_variable('b_f', shape=[1, D_out],
                                        initializer=tf.compat.v1.constant_initializer(2.5),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'W_c': tf.compat.v1.get_variable('W_c', shape=[D_in, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'U_c': tf.compat.v1.get_variable('U_c', shape=[D_out, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'b_c': tf.compat.v1.get_variable('b_c', shape=[1, D_out],
                                        initializer=tf.compat.v1.constant_initializer(0.0),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'W_o': tf.compat.v1.get_variable('W_o', shape=[D_in, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'U_o': tf.compat.v1.get_variable('U_o', shape=[D_out, D_out],
                                        initializer=tf.compat.v1.initializers.random_uniform(minval=-r, maxval=r),
                                        trainable=True,
                                        dtype=tf.float32,
                                        ),
                 'b_o': tf.compat.v1.get_variable('b_o', shape=[1, D_out],
    ...
    # train end-to-end
    ##################
    def train_model(self, model_folder, dev_set, file_results, file_tmp, dir_results):
        ...
        for epoch in range(epochs):
            ...
            # training set
            docs_L_tr, docs_R_tr, labels_a_tr, labels_f_tr = resample_pairs_train(model_folder, file_results)
            # shuffle (again)
            ...
            print('Training...')
            for i in tqdm(range(num_batches_tr)):
