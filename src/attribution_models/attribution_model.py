...
class AttributionModel(ABC):
    ...
    def __init__(self, args, parameter_set):
        ...
        if args.test:
            self.test_df = pd.read_csv(args.test_files[0])
            # assert set(self.test_df['author']) == (set(self.train_df['author']))
        
        author_list = set(self.train_df['author'])
        ...
        if hasattr(self, 'train_df'):
            self.train_df = self.train_df.apply(lambda row: update_authors(row, self.author_to_author_id), axis=1)
        if hasattr(self, 'test_df'):
            self.test_df = self.test_df.apply(lambda row: update_authors(row, self.author_to_author_id), axis=1)
    ...
    def evaluate(self, df_name=None):
        ...
        """
        all_scores = self.evaluate_internal(self.train_df, self.test_df, df_name=df_name)
